...
*   **Subconscious (Pray + Dream)** (`src/evolution/subconscious/`): Two cron jobs — **Pray** (2 AM) contemplates the saint of the day and may stage a skill; **Dream** (3 AM) ingests 24h memory and may stage a lateral skill. Review with `/evolve approve|reject` in the TUI. Soul files are **not** auto-rewritten.

Manual run: `PYTHONPATH=src .venv/bin/python3 -m evolution.subconscious all`
*   **Self-Authorship**: Uses `StarkNIMChatClient` with house-party failover. Approved skills sync to GitHub via narrow `sync_evolution_artifacts()`.
